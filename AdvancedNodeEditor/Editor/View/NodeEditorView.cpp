...
namespace NodeEditorCore {
    NodeEditorView::NodeEditorView(std::shared_ptr<INodeEditorController> controller)
        ...
        m_controller->addEventListener(EventType::NodeCreated, [this](const Event &event) {
            ...
            if (modelNode) {
                m_coreEditor->addNode(modelNode->name, modelNode->type,
                                      NodeEditorCore::Vec2(modelNode->position.x, modelNode->position.y));
            }
        });
    ...
    void NodeEditorView::centerViewWithSize(float windowWidth, float windowHeight) {
        m_coreEditor->centerViewWithSize(windowWidth, windowHeight);
        NodeEditorCore::Vec2 pos = m_coreEditor->getViewPosition();
        m_viewPosition = Vec2(pos.x, pos.y);
    }

    void NodeEditorView::centerOnNodeWithSize(int nodeId, float windowWidth, float windowHeight) {
        m_coreEditor->centerOnNodeWithSize(nodeId, windowWidth, windowHeight);
        NodeEditorCore::Vec2 pos = m_coreEditor->getViewPosition();
        m_viewPosition = Vec2(pos.x, pos.y);
    }
    ...
    int NodeEditorView::addLayer(const std::string &name, int zOrder,

...
std::vector<Vec2> NodeBoundingBoxManager::findPathAroundNodes(const Vec2& start, const Vec2& end, float padding) const {
    // Créer une copie des boîtes avec padding
    ...
    // Si la ligne directe ne coupe aucune boîte, la retourner
    ...
    // Sinon, trouver un chemin en contournant les obstacles
...
Vec2 NodeBoundingBoxManager::findNearestPointOnLine(const Vec2& point, const Vec2& lineStart, const Vec2& lineEnd) const {
...
bool NodeBoundingBoxManager::isPointValid(const Vec2& point, const std::vector<BoundingBox>& boxes, float padding) const {
...
std::vector<Vec2> NodeBoundingBoxManager::findSimplePath(const Vec2& start, const Vec2& end,
                                                      ...
                                                      float padding) const {
    // Implémentation simplifiée pour trouver un chemin en contournant les obstacles
    // Cette version utilise une approche à points de contrôle pour les styles de connexion Metro

    // Différentes directions à essayer
    const Vec2 directions[] = {
        // Droite-haut, droite-bas, gauche-haut, gauche-bas
    ...
    if (preferHorizontal) {
        // Essayer un chemin avec segment horizontal d'abord
        middle = Vec2(start.x + diffX * 0.5f, start.y);

        // Vérifier si ce chemin est valide
        if (!isLineIntersectingAnyBox(start, middle) && !isLineIntersectingAnyBox(middle, end)) {
            return {start, middle, end};
        }

        // Essayer un chemin avec segment vertical d'abord
    ...
    // Essayer un chemin à 3 segments
    ...
    // Essayer d'autres points de passage
    ...
    // En dernier recours, retourner une ligne directe
    return {start, end};
}

...
namespace NodeEditorCore {
    ...
    ImVec2 MinimapManager::graphToMinimap(const Vec2 &graphPos, const ImVec2 &minimapPos,
                                          const ImVec2 &minimapSize) const {
        Vec2 graphExtent = m_viewMax - m_viewMin;
        float graphWidth = graphExtent.x;
        float graphHeight = graphExtent.y;

        float x = minimapPos.x + m_config.padding + (graphPos.x - m_viewMin.x) / graphWidth * (
                      minimapSize.x - 2 * m_config.padding);
        float y = minimapPos.y + m_config.padding + (graphPos.y - m_viewMin.y) / graphHeight * (
                      minimapSize.y - 2 * m_config.padding);
    ...
    bool MinimapManager::draw(ImDrawList *drawList, const ImVec2 &canvasPos, const ImVec2 &canvasSize) {
        ...
        drawList->AddRectFilled(
            ...
            ImGui::ColorConvertFloat4ToU32(ImVec4(m_config.backgroundColor.r, m_config.backgroundColor.g,
                                                  ...
                                                  m_config.backgroundColor.a * m_config.opacity)),
            4.0f
        );
        ...
        if (m_config.showBorder) {
            drawList->AddRect(
                ...
                ImGui::ColorConvertFloat4ToU32(ImVec4(m_config.borderColor.r, m_config.borderColor.g,
                                                      m_config.borderColor.b,
                                                      m_config.borderColor.a * m_config.opacity)),
                4.0f,
                ImDrawFlags_None,
                1.5f
            );
        ...
        drawList->AddRect(
            ...
            ImGui::ColorConvertFloat4ToU32(ImVec4(m_config.viewportColor.r, m_config.viewportColor.g,
                                                  m_config.viewportColor.b,
                                                  m_config.viewportColor.a * m_config.opacity)),
            0.0f,
            ImDrawFlags_None,
            2.0f
        );

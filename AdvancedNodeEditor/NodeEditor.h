        ...
        std::vector<int> getEvaluationOrder() {
            ...
            for (const auto &connection: m_state.connections) {
                if (m_state.currentSubgraphId >= 0) {
                    bool isInCurrentSubgraph = false;
                    auto subgraphConnections = getConnectionsInSubgraph(m_state.currentSubgraphId);
                    if (std::find(subgraphConnections.begin(), subgraphConnections.end(),
                                  connection.id) != subgraphConnections.end()) {
                        isInCurrentSubgraph = true;
                    }

                    if (!isInCurrentSubgraph) {
                        continue;
                    }
                } else if (connection.getSubgraphId() != -1) {
                    continue;
                }

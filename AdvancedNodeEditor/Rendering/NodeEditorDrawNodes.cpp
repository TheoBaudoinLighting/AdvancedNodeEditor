...
namespace NodeEditorCore {
    void NodeEditor::drawNodes(ImDrawList *drawList, const ImVec2 &canvasPos) {
        ...
        for (Node *nodePtr: visibleNodes) {
            ...
            const bool hasHeaderButtons = nodeHasHeaderButtons(node);
            ...
            if (nodeAnimState.executionPulse > 0.0f) {
                executionPulseIntensity = std::sin(nodeAnimState.executionPulse * Math::TWO_PI) * 0.5f + 0.5f;
            }
            ...
            const int validationSeverity = node.getMetadata<int>("validationSeverity", 0);

            if (validationSeverity > 0) {
                activeBorderColor = validationSeverity >= 3
                    ? IM_COL32(255, 82, 66, 255)
                    : validationSeverity == 2
                          ? IM_COL32(255, 190, 70, 255)
                          : IM_COL32(120, 170, 255, 220);
                borderThickness = std::max(borderThickness, 2.0f * m_state.viewScale);
            }
            ...
            for (int segment = 0; segment < segments; segment++) {
                ...
                ImU32 segmentBorderColor = node.selected
                                               ? actualSelectedColor
                                               : ImGui::ColorConvertFloat4ToU32(gradientBorderColor);
            ...
            bool hoverDisableBar = mousePos.x >= buttonLayout.disableMin.x && mousePos.x <= buttonLayout.disableMax.x &&
                                   mousePos.y >= buttonLayout.disableMin.y && mousePos.y <= buttonLayout.disableMax.y;

            bool hoverTemplateBar = mousePos.x >= buttonLayout.templateMin.x &&
                                    mousePos.x <= buttonLayout.templateMax.x &&
                                    mousePos.y >= buttonLayout.templateMin.y &&
                                    mousePos.y <= buttonLayout.templateMax.y;

            bool hoverFlagBar = mousePos.x >= buttonLayout.flagMin.x && mousePos.x <= buttonLayout.flagMax.x &&
            ...
            if (hasHeaderButtons) {
                drawList->AddRectFilled(
                    ImVec2(buttonLayout.rightSideX - buttonLayout.separatorWidth, nodePos.y),
                    ImVec2(buttonLayout.rightSideX, nodePos.y + buttonLayout.buttonHeight),
                    accentColor
                );

                drawList->AddRectFilled(
                    buttonLayout.disableMin,
                    buttonLayout.disableMax,
                    disableColor
                );

                drawList->AddRectFilled(
                    ImVec2(buttonLayout.rightSideX + buttonLayout.buttonWidth, nodePos.y),
                    ImVec2(buttonLayout.rightSideX + buttonLayout.buttonWidth + buttonLayout.separatorWidth,
                           nodePos.y + buttonLayout.buttonHeight),
                    accentColor
                );

                drawList->AddRectFilled(
                    buttonLayout.templateMin,
                    buttonLayout.templateMax,
                    templateColor
                );

                drawList->AddRectFilled(
                    ImVec2(buttonLayout.rightSideX + 2.0f * buttonLayout.buttonWidth + buttonLayout.separatorWidth,
                           nodePos.y),
                    ImVec2(buttonLayout.rightSideX + 2.0f * buttonLayout.buttonWidth +
                           2.0f * buttonLayout.separatorWidth, nodePos.y + buttonLayout.buttonHeight),
                    accentColor
                );

                drawList->AddRectFilled(
                    buttonLayout.flagMin,
                    buttonLayout.flagMax,
                    flagColor,
            ...
            if (validationSeverity > 0) {
                const ImU32 badgeColor = validationSeverity >= 3
                    ? IM_COL32(255, 82, 66, 255)
                    : validationSeverity == 2
                          ? IM_COL32(255, 190, 70, 255)
                          : IM_COL32(120, 170, 255, 220);
                const std::string badge = node.getMetadata<std::string>("validationBadge", "!");
                const int validationCount = node.getMetadata<int>("validationCount", 1);
                const std::string badgeText = validationCount > 1
                    ? badge + std::to_string(std::min(validationCount, 9))
                    : badge;
                const ImVec2 badgeTextSize = ImGui::CalcTextSize(badgeText.c_str());
                const float badgePaddingX = 3.0f * m_state.viewScale;
                const float badgePaddingY = 1.0f * m_state.viewScale;
                const ImVec2 badgeMax(nodePos.x + nodeSize.x - 4.0f * m_state.viewScale,
                                      nodePos.y + 4.0f * m_state.viewScale + badgeTextSize.y + badgePaddingY * 2.0f);
                const ImVec2 badgeMin(badgeMax.x - badgeTextSize.x - badgePaddingX * 2.0f,
                                      nodePos.y + 4.0f * m_state.viewScale);
                drawList->AddRectFilled(badgeMin, badgeMax, badgeColor, 3.0f * m_state.viewScale);
                drawList->AddRect(badgeMin, badgeMax, IM_COL32(20, 20, 24, 220), 3.0f * m_state.viewScale, 0, 1.0f);
                drawList->AddText(
                    ImVec2(badgeMin.x + badgePaddingX, badgeMin.y + badgePaddingY),
            ...
            if (nodeAnimState.justConnected && nodeAnimState.connectionGlow > 0.0f) {
                ...
                for (int i = 0; i < particleCount; i++) {
                    float angle = nodeAnimState.connectionGlowAngle + (i * Math::TWO_PI / particleCount);
            ...
            if (m_debugMode && hasHeaderButtons) {
                const NodeHeaderButtonLayout debugButtonLayout = getNodeHeaderButtonLayout(nodePos, nodeSize);

                drawList->AddRect(debugButtonLayout.disableMin, debugButtonLayout.disableMax,
                                  IM_COL32(255, 255, 0, 200), 0.0f, 0, 2.0f);
                drawList->AddRect(debugButtonLayout.templateMin, debugButtonLayout.templateMax,
                                  IM_COL32(255, 0, 255, 200), 0.0f, 0, 2.0f);
                drawList->AddRect(debugButtonLayout.flagMin, debugButtonLayout.flagMax,
                                  IM_COL32(0, 255, 255, 200), 0.0f, 0, 2.0f);

                char debugText[128];
                sprintf(debugText, "Buttons: D[%.0f-%.0f] T[%.0f-%.0f] F[%.0f-%.0f]",
                        debugButtonLayout.disableMin.x, debugButtonLayout.disableMax.x,
                        debugButtonLayout.templateMin.x, debugButtonLayout.templateMax.x,
                        debugButtonLayout.flagMin.x, debugButtonLayout.flagMax.x);
                drawList->AddText(ImVec2(nodePos.x, nodePos.y - 35), IM_COL32(255, 255, 255, 255), debugText);
            }

            drawNodePins(drawList, node, nodePos, nodeSize, canvasPos);

            if (validationSeverity > 0 && isHovered) {
                const std::string tooltip = node.getMetadata<std::string>("validationTooltip", "");
                if (!tooltip.empty()) {

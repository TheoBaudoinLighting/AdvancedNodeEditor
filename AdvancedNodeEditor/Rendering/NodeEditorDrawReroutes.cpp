...
namespace NodeEditorCore {
    namespace {
        ...
        bool hasSameConnectionSource(const Connection &lhs, const Connection &rhs) {
            if (!lhs.startNodeUuid.empty() && !rhs.startNodeUuid.empty() &&
                !lhs.startPinUuid.empty() && !rhs.startPinUuid.empty()) {
                return lhs.startNodeUuid == rhs.startNodeUuid &&
                       lhs.startPinUuid == rhs.startPinUuid;
            }

            return lhs.startNodeId == rhs.startNodeId &&
                   lhs.startPinId == rhs.startPinId;
        }

        bool hasSameConnectionTarget(const Connection &lhs, const Connection &rhs) {
            if (!lhs.endNodeUuid.empty() && !rhs.endNodeUuid.empty() &&
                !lhs.endPinUuid.empty() && !rhs.endPinUuid.empty()) {
                return lhs.endNodeUuid == rhs.endNodeUuid &&
                       lhs.endPinUuid == rhs.endPinUuid;
            }

            return lhs.endNodeId == rhs.endNodeId &&
                   lhs.endPinId == rhs.endPinId;
        }
    }
    ...
    std::pair<ImVec2, ImVec2> NodeEditor::calculateBezierControlPoints(
        ...
        const float distance = Math::distance(
            Vec2(segStart.x, segStart.y),
            Vec2(segEnd.x, segEnd.y));
    ...
    float NodeEditor::getDistanceToConnection(const Connection &connection, const ImVec2 &mousePos,
                                              const ImVec2 &canvasPos, int &insertIndex) const {
        ...
        for (size_t i = 0; i < pathPoints.size() - 1; i++) {
            ...
            switch (style) {
                case ConnectionStyleManager::ConnectionStyle::Bezier: {
                    auto [cp1, cp2] = calculateBezierControlPoints(segStart, segEnd, segmentStartInput, segmentEndInput,
                                                                   tension);
    ...
    float NodeEditor::getDistanceToBezierCubic(const ImVec2 &point, const ImVec2 &p0, const ImVec2 &p1,
                                               const ImVec2 &p2, const ImVec2 &p3) const {
        float minDistance = FLT_MAX;
    ...
    float NodeEditor::getDistanceToLineSegment(const ImVec2 &point, const ImVec2 &lineStart,
                                               const ImVec2 &lineEnd) const {
    ...
    int NodeEditor::addReroute(int connectionId, const Vec2 &position, int insertIndex,
        ...
        auto isFlowConnection = [&](const Connection &candidate) {
            const Node *startNode = getNode(candidate.startNodeId);
            const Pin *startPin = startNode ? startNode->findPin(candidate.startPinId) : nullptr;
            return startPin && isRuntimeFlowPin(*startPin);
        };

        const bool connectionIsFlow = isFlowConnection(*connection);

        const UUID resolvedKnotUuid = !knotUuid.empty() ? knotUuid : uuid;
        if (!resolvedKnotUuid.empty()) {
            for (const auto &existingReroute: m_reroutes) {
                if (rerouteKnotKey(existingReroute) != resolvedKnotUuid) {
                    continue;
                }

                const Connection *existingConnection = getConnection(existingReroute.connectionId);
                if (!existingConnection) {
                    continue;
                }

                const bool existingConnectionIsFlow = isFlowConnection(*existingConnection);
                if (existingConnectionIsFlow != connectionIsFlow) {
                    return -1;
                }

                const bool compatibleKnotEndpoint = connectionIsFlow
                                                        ? hasSameConnectionTarget(*existingConnection, *connection)
                                                        : hasSameConnectionSource(*existingConnection, *connection);
                if (!compatibleKnotEndpoint) {
                    return -1;
                }
            }
        }
        ...
        Reroute newReroute(rerouteId, connectionId, position, insertIndex, uuid, knotUuid);
    ...
    void NodeEditor::removeReroute(int rerouteId) {
        ...
        if (it != m_reroutes.end()) {
            const UUID knotUuid = rerouteKnotKey(*it);
            std::unordered_map<int, std::vector<int>> removedIndicesByConnection;

            for (const auto &reroute: m_reroutes) {
                if (rerouteKnotKey(reroute) == knotUuid) {
                    removedIndicesByConnection[reroute.connectionId].push_back(reroute.index);
                }
            }

            m_reroutes.erase(
                std::remove_if(m_reroutes.begin(), m_reroutes.end(),
                               [&](const Reroute &r) { return rerouteKnotKey(r) == knotUuid; }),
                m_reroutes.end());

            for (auto &[connectionId, removedIndices]: removedIndicesByConnection) {
                std::sort(removedIndices.begin(), removedIndices.end());
                for (auto &reroute: m_reroutes) {
                    if (reroute.connectionId != connectionId) continue;

                    int shift = 0;
                    for (int removedIndex: removedIndices) {
                        if (reroute.index > removedIndex) {
                            ++shift;
                        }
                    }
                    reroute.index -= shift;
    ...
    void NodeEditor::drawReroutes(ImDrawList *drawList, const ImVec2 &canvasPos) {
        std::unordered_set<UUID, UUIDHash> drawnKnots;

        for (const auto &reroute: m_reroutes) {
            const UUID &knotUuid = rerouteKnotKey(reroute);
            if (drawnKnots.count(knotUuid) != 0) continue;
    ...
    void NodeEditor::drawSingleReroute(ImDrawList *drawList, const Reroute &reroute, const ImVec2 &canvasPos) {
        ...
        drawList->AddCircle(center, outerRadius, m_rerouteStyle.borderColor, 32,
                            m_rerouteStyle.borderWidth * m_state.viewScale);

        drawList->AddCircleFilled(center, innerRadius, innerColor);
        drawList->AddCircle(center, innerRadius, m_rerouteStyle.borderColor, 32,
                            m_rerouteStyle.borderWidth * m_state.viewScale);
    }
    ...
    void NodeEditor::drawRerouteDebugInfo(ImDrawList *drawList, const ImVec2 &canvasPos) {
        ...
        for (const auto &reroute: m_reroutes) {
            ...
            char debugText[96];
            sprintf(debugText, "R%d[%d] x%d", reroute.id, reroute.index, members);
    ...
    void NodeEditor::updateRerouteHover(const ImVec2 &mousePos, const ImVec2 &canvasPos) {
        ...
        for (const auto &reroute: m_reroutes) {
            ...
            const UUID &knotUuid = rerouteKnotKey(reroute);
            if (!testedKnots.insert(knotUuid).second) {
                continue;
            }
    ...
    void NodeEditor::processRerouteDrag(const ImVec2 &mousePos) {
        ...
        for (auto &member: m_reroutes) {
            if (rerouteKnotKey(member) == knotUuid) {
                member.position = newCanvasPos;
            }
        }
    }

    RerouteHitZone NodeEditor::getRerouteHitZone(const Reroute &reroute, const ImVec2 &mousePos,
        ...
        float distance = Math::distance(
            Vec2(rerouteScreenPos.x, rerouteScreenPos.y),
    ...
    int NodeEditor::findRerouteAtPosition(const ImVec2 &mousePos, const ImVec2 &canvasPos,
                                          RerouteHitZone &hitZone) const {
        std::unordered_set<UUID, UUIDHash> testedKnots;
    ...
    std::vector<int> NodeEditor::getSelectedReroutes() const {
        ...
        std::unordered_set<UUID, UUIDHash> selectedKnots;

        for (const auto &reroute: m_reroutes) {
            const UUID &knotUuid = rerouteKnotKey(reroute);
            if (reroute.selected && selectedKnots.insert(knotUuid).second) {
    ...
    std::vector<ImVec2> NodeEditor::getConnectionPathWithReroutesForDetection(
        const Connection &connection, const ImVec2 &canvasPos) const {
        const Node *startNode = getNode(connection.startNodeId);
        const Node *endNode = getNode(connection.endNodeId);
